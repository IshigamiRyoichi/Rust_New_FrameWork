...
fn write_model_file(node_and_arc: Count) -> std::io::Result<()> {
    let mut file = OpenOptions::new()
        .read(true)
        .write(true)
        .open("src/test/data_json/model_file.json")?;
    // println!("file:{:#?}",file);
...
fn write_input_file(input_list : BTreeMap<String, BTreeMap<String, Input_Data>>)  -> std::io::Result<()> {
    let mut file = OpenOptions::new()
        .read(true)
        .write(true)
        .open("src/test/data_json/input_file.json")?;
...
fn anarisis_controller(file_name: &Path){
    // println!("controller");
}
...
fn main(){
    ...
    // htmlファイルをリスト化
    let html_file_list = glob("./templates/*.html").unwrap()
    .map(|e| e.unwrap())
    .collect::<Vec<_>>();
